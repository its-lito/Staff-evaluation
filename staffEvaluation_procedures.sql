...
call employee_eval('Ahmet', 'Mobasher-Hirs');
...
BEGIN
    ...
    create temporary table temp 
    (Status VARCHAR(40), Employee varchar(12),EvID INT, Grade INT);
	...
	WHILE(finishedFlag=0) DO
		...
		IF completed_ev = 0	THEN
			...
			ELSE
				SET final_result=0;
				INSERT into temp values ('Evaluation in progress', empl, _id, final_result);
                #SELECT "Evaluation in progress" AS 'Status', empl AS 'Employee', jobid AS 'Job ID';
				
			END IF;
		ELSE
			select grade into final_result from evaluationresult where EVid = _id;
			INSERT into temp values ('Evaluation completed', empl, _id, final_result);
		END IF;
